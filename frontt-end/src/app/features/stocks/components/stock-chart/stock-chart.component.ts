...
export class StockChartComponent implements OnInit, OnDestroy {
  ...
  public selectedSymbol: string = 'AAPL';
  public companies = [
  ...
  private pollingInterval = 12000000;

  constructor(private stockService: StockService,private companyService: CompanyService, private datePipe: DatePipe) {
    ...
    this.chartOptions = {
      ...
      zoom: {
        enabled: true,
        type: 'x', // Horizontal zooming only
    ...
    this.tooltip = {
      ...
      y: {
        formatter: (val, { seriesIndex, dataPointIndex, w }) => {
          ...
          if (seriesName === this.selectedSymbol) { // Candlestick (OHLC)
    ...
    this.title = {
      text: `${this.selectedSymbol} Hourly Stock Price`,
      align: 'left',
      style: { 
        fontSize: '24px',
        fontWeight: 600,
  ...
  onChartFocus() {
    // Disable page scrolling when chart is focused
    document.body.style.overflow = 'hidden';
  }

  onChartBlur() {
    // Re-enable page scrolling when chart loses focus
  ...
  // In your StockChartComponent
testFinnhubConnection() {
  this.stockService.testApiConnection().subscribe({
    next: (data) => console.log('Test Success:', data),
    error: (err) => console.error('Test Failed:', err)
  });
}

  onSelectCompany(symbol: string) {
    this.selectedSymbol = symbol;
    this.getStockData(symbol);
  }

  ngOnInit(): void {
    this.getStockData(this.selectedSymbol); // Initial fetch
    this.startPolling(); // Start polling for updates
    this.updateXAxis(); // Update X-axis based on selected time range
  }
  ...
  onTimeRangeChange(range: '1D' | '1W' | '1M' | '1Y') {
    ...
    this.updateXAxis();
    this.getStockData(this.selectedSymbol);
  }

  private startPolling() {
    // this.pollingSubscription = interval(this.pollingInterval).subscribe(() => {
    //   this.stockService.getStockData(this.selectedSymbol).subscribe({
    //     next: (data) => {
    //       this.appendNewData(data); // Append only new data points
    //     },
    //     error: (err) => {
  ...
  private appendNewData(newData: any[]) {
    ...
    // Filter only new data points (after the last timestamp)
